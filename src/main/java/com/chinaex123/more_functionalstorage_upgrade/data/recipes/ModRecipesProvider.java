...
public class ModRecipesProvider extends RecipeProvider {
    /** 物品注册表查询器，用于构建配方 */
    private final HolderGetter<Item> itemRegistryLookup;
    /** 配方输出 */
    ...
    public ModRecipesProvider(HolderLookup.Provider provider, RecipeOutput output) {
        ...
        this.itemRegistryLookup = provider.lookupOrThrow(Registries.ITEM);
        this.output = output;
    }
     ...
     * 配方生成器运行器。
     * <p>
     * 用于在数据生成入口创建 {@link ModRecipesProvider} 实例。
     */
    public static class Runner extends RecipeProvider.Runner {
        ...
        public @NotNull String getName() {
            return MoreFunctionalStorageUpgrade.MOD_ID;
        }
    }

    /** 获取物品注册表查询器，供子 Provider 构建配方使用 */
    public HolderGetter<Item> getItemLookup() {
        return itemRegistryLookup;
    }

    /** 获取配方输出，供子 Provider 直接保存配方使用 */
    public RecipeOutput getOutput() {
        return output;
    }

    /** 将标签转换为 Ingredient 所需的 HolderSet 形式 */
    public Ingredient tagIngredient(TagKey<Item> tag) {
        return Ingredient.of(itemRegistryLookup.getOrThrow(tag));
    }
    ...
    public static Fluid getFluid(String fluidId) {
        return BuiltInRegistries.FLUID.get(Identifier.parse(fluidId))
                .map(Holder.Reference::value)
                .orElse(Fluids.EMPTY);
    }
     ...
     * @return 解锁条件
     */
    public Criterion<InventoryChangeTrigger.TriggerInstance> has(Item item) {
     ...
     * @param c          材料 C
     */
    protected void tier1Recipe(Item result,
                               Ingredient center,
                               ItemLike unlockItem,
                               Ingredient b,
                               Ingredient c) {
        ShapedRecipeBuilder.shaped(itemRegistryLookup, RecipeCategory.MISC, result)
                .pattern("BCB")
                .pattern("BAB")
                .pattern("BCB")
                .define('A', center)
     ...
     * @param d      材料 D
     */
    public void tier1Recipe(Item result,
                            TagKey<Item> center,
                            Ingredient b,
                            Ingredient c,
                            Ingredient d) {
     ...
     * @param d          材料 D
     */
    protected void tier1RecipeCDC(Item result,
                                  Ingredient center,
                                  ItemLike unlockItem,
                                  Ingredient b,
                                  Ingredient c,
     ...
     * @param e          材料 E
     */
    public void tier1RecipeHoney(Item result,
                                 Ingredient center,
                                 ItemLike unlockItem,
                                 Ingredient b,
                                 Ingredient c,
     ...
     * @param c        材料 C
     */
    protected void tieredUpgrade(Item result,
                                 Item previous,
                                 Ingredient b,
                                 Ingredient c) {
        ShapedRecipeBuilder.shaped(itemRegistryLookup, RecipeCategory.MISC, result)
     ...
     */
    protected void smithingTo4(Item result,
                               Item previous,
                               String baseName) {
        SmithingTransformRecipeBuilder.smithing(
                ...
                .save(output, String.valueOf(Identifier.fromNamespaceAndPath(
     ...
     * @param baseName 配方基础名称，用于生成注册 ID
     */
    protected void smithingTo5(Item result,
                               Item previous,
                               String baseName) {
        SmithingTransformRecipeBuilder.smithing(
     ...
     * @param baseName 配方基础名称，用于生成注册 ID
     */
    public void fullGeneratorUpgradeChain(Item upgrade1,
                                          Item upgrade2,
                                          Item upgrade3,
                                          Item upgrade4,
                                          Item upgrade5,
                                          String baseName) {
        // 2 级：金锭 + 金块
        tieredUpgrade(upgrade2, upgrade1,
                Ingredient.of(itemRegistryLookup.getOrThrow(Tags.Items.INGOTS_GOLD)),
                Ingredient.of(itemRegistryLookup.getOrThrow(Tags.Items.STORAGE_BLOCKS_GOLD)));

        // 3 级：钻石 + 钻石块
        tieredUpgrade(upgrade3, upgrade2,
                Ingredient.of(itemRegistryLookup.getOrThrow(Tags.Items.GEMS_DIAMOND)),
                Ingredient.of(itemRegistryLookup.getOrThrow(Tags.Items.STORAGE_BLOCKS_DIAMOND)));

        // 4 级与 5 级：锻造台配方
        smithingTo4(upgrade4, upgrade3, baseName);
        smithingTo5(upgrade5, upgrade4, baseName);
    }

    @Override
    protected void buildRecipes() {
        CustomUpgradeRecipesProvider.buildRecipes(this);
//        CreateRecipesProvider.buildRecipes(this);
        ForbiddenArcanusRecipesProvider.buildRecipes(this);
    }
}